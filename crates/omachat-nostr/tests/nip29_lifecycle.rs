...
fn metadata(relay_secret: &[u8; 32], group: &str) -> GroupMetadata {
    GroupMetadata::verify(
        signed(relay_secret, NOW - 90, 39000, vec![tag(&["d", group])]),
        &pubkey(relay_secret),
        NOW,
        &limits(),
    )
...
fn accepted_admin(request: GroupLifecycleRequest) -> AcceptedLifecycleAction {
    let roster = admins(&RELAY_SECRET, request.group_id(), request.author());
    AcceptedLifecycleAction::by_administrator(request, &roster, &pubkey(&RELAY_SECRET))
        .expect("accepted")
}

fn accepted_creation(request: GroupLifecycleRequest) -> AcceptedLifecycleAction {
    let metadata = metadata(&RELAY_SECRET, request.group_id());
    AcceptedLifecycleAction::by_relay_metadata(request, &metadata, &pubkey(&RELAY_SECRET))
...
#[test]
fn unauthorized_actions_have_no_accepted_effect() {
    let relay = pubkey(&RELAY_SECRET);
    let roster = admins(&RELAY_SECRET, "omarchy", &pubkey(&MODERATOR_SECRET));

    for request in [
        create(&AGENT_SECRET, "omarchy", NOW - 3),
        delete(&AGENT_SECRET, "omarchy", NOW - 2),
        invite(&AGENT_SECRET, "omarchy", NOW - 1, "code"),
    ] {
        assert_eq!(
            AcceptedLifecycleAction::by_administrator(request, &roster, &relay).err(),
            Some(LifecycleAuthorizationError::RequesterNotAdministrator)
        );
    }

    // Relay metadata evidences creation only, and only for that group and relay.
    let omarchy = metadata(&RELAY_SECRET, "omarchy");
    assert_eq!(
        AcceptedLifecycleAction::by_relay_metadata(
            delete(&AGENT_SECRET, "omarchy", NOW - 2),
            &omarchy,
            &relay
        )
        .err(),
        Some(LifecycleAuthorizationError::MetadataOnlyProvesCreation)
    );
    assert_eq!(
        AcceptedLifecycleAction::by_relay_metadata(
            create(&AGENT_SECRET, "other", NOW - 3),
            &omarchy,
            &relay
        )
        .err(),
        Some(LifecycleAuthorizationError::MetadataGroupMismatch)
    );
    let foreign = metadata(&OTHER_RELAY_SECRET, "omarchy");
    assert_eq!(
        AcceptedLifecycleAction::by_relay_metadata(
            create(&AGENT_SECRET, "omarchy", NOW - 3),
            &foreign,
        ...
        .err(),
        Some(LifecycleAuthorizationError::MetadataRelayMismatch)
    );

    // Cross-group invitation: the roster that lists the requester is for another group.
    let elsewhere = admins(&RELAY_SECRET, "elsewhere", &pubkey(&AGENT_SECRET));
    assert_eq!(
        AcceptedLifecycleAction::by_administrator(
            invite(&AGENT_SECRET, "omarchy", NOW - 1, "code"),
            &elsewhere,
            &relay
        )
        .err(),
        Some(LifecycleAuthorizationError::RosterGroupMismatch)
    );
    // Another relay's roster is not this relay's policy.
    let foreign_roster = admins(&OTHER_RELAY_SECRET, "omarchy", &pubkey(&AGENT_SECRET));
    assert_eq!(
        AcceptedLifecycleAction::by_administrator(
            invite(&AGENT_SECRET, "omarchy", NOW - 1, "code"),
            &foreign_roster,
        ...
        .err(),
        Some(LifecycleAuthorizationError::RosterRelayMismatch)
    );
...
fn creation_is_idempotent_and_conflicts_resolve_deterministically() {
    let first = accepted_creation(create(&AGENT_SECRET, "omarchy", NOW - 3));
    let rival = accepted_creation(create(&MODERATOR_SECRET, "omarchy", NOW - 2));
    ...
    assert_eq!(
        ...
        &LifecycleAuthority::RelayMetadata
    );
...
fn accepted_deletion_is_terminal_and_preserves_history() {
    let creation = accepted_creation(create(&AGENT_SECRET, "omarchy", NOW - 5));
    let code = accepted_admin(invite(&MODERATOR_SECRET, "omarchy", NOW - 4, "early"));
    let deletion = accepted_admin(delete(&MODERATOR_SECRET, "omarchy", NOW - 3));
    let late_invite = accepted_admin(invite(&MODERATOR_SECRET, "omarchy", NOW - 2, "late"));
    let recreate = accepted_creation(create(&AGENT_SECRET, "omarchy", NOW - 1));
    ...
    assert_eq!(
        ...
        &LifecycleAuthority::Administrator {
...
fn invitations_are_scoped_and_are_not_membership() {
    let mut state = state();
    state
        .apply_accepted(&accepted_creation(create(
            &AGENT_SECRET,
            "omarchy",
            NOW - 5,
        )))
        .expect("create");
    let issued = accepted_admin(invite(&MODERATOR_SECRET, "omarchy", NOW - 4, "welcome"));
    state.apply_accepted(&issued).expect("invite");
    let repeat = accepted_admin(invite(&MODERATOR_SECRET, "omarchy", NOW - 3, "welcome"));
    ...
    state
        .apply_accepted(&accepted_admin(delete(&MODERATOR_SECRET, "ghost", NOW - 2)))
        .expect("recorded");
    state
        .apply_accepted(&accepted_admin(invite(
            &MODERATOR_SECRET,
            "ghost",
            NOW - 1,
            "boo",
...
fn multi_relay_duplicates_and_foreign_relays_are_handled() {
    ...
    let via_primary = accepted_creation(request.clone());
    let via_mirror = accepted_creation(request.clone());
    let mut state = state();
    ...
    let foreign = AcceptedLifecycleAction::by_relay_metadata(
        request,
        &metadata(&OTHER_RELAY_SECRET, "omarchy"),
        &other_relay,
    )
...
fn reduction_is_independent_of_ingestion_order() {
    let inputs = [
        accepted_creation(create(&AGENT_SECRET, "omarchy", NOW - 9)),
        accepted_admin(invite(&MODERATOR_SECRET, "omarchy", NOW - 8, "a")),
        accepted_creation(create(&MODERATOR_SECRET, "omarchy", NOW - 7)),
        accepted_creation(create(&AGENT_SECRET, "linux", NOW - 6)),
        accepted_admin(delete(&MODERATOR_SECRET, "omarchy", NOW - 5)),
        accepted_admin(invite(&MODERATOR_SECRET, "omarchy", NOW - 4, "b")),
        accepted_admin(invite(&AGENT_SECRET, "linux", NOW - 3, "c")),
        accepted_creation(create(&AGENT_SECRET, "omarchy", NOW - 2)),
    ];
    let orders = [
        [0, 1, 2, 3, 4, 5, 6, 7],
        [7, 6, 5, 4, 3, 2, 1, 0],
