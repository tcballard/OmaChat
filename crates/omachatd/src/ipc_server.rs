...
fn bind_private_socket(socket_path: &Path) -> Result<UnixListener, ServerError> {
    ...
    let staging_directory = parent.join(format!(".{file_name}.{}", hex::encode(nonce)));
    fs::DirBuilder::new()
        .mode(0o700)
...
async fn serve_client<H: RequestHandler>(
    ...
    let mut event_receiver: Option<mpsc::Receiver<Event>> = None;
    ...
    loop {
        ...
        tokio::select! {
            ...
            read = stream.read(&mut read_buffer) => {
                ...
                for request in requests {
                    ...
                    let outcome = match &request.command {
                        ...
                        Command::Subscribe { topics: requested } => {
                            topics = requested.clone();
                            event_receiver = Some(events.subscribe());
            ...
            event = async { event_receiver.as_mut().expect("subscribed").recv().await }, if event_receiver.is_some() => {
                let Some(event) = event else { return Ok(()) };
                if !topics.contains(&event.topic) { continue; }
