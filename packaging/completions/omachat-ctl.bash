_omachat_ctl() {
  ...
  if [[ $COMP_CWORD -eq 1 ]]; then
    COMPREPLY=($(compgen -W '--socket status fingerprint join leave send join-room leave-room rooms room-members panic' -- "$current"))
  elif [[ ${COMP_WORDS[1]} == status || ${COMP_WORDS[1]} == rooms || ${COMP_WORDS[1]} == room-members ]]; then
